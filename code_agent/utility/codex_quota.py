#!/usr/bin/env python3
"""codex-quota — Codex quota checker reading ~/.code-agent/codex-auth.json.
...
DEFAULT_FILE = os.path.expanduser("~/.code-agent/codex-auth.json")

# ── colour helpers (mirrors cliproxyapi/quota) ──────────────────────────────────
try:
    ...
    for idx, cred in enumerate(data.get("credentials") or []):
        ...
        for key, val in limits.items():
            ...
            windows.append({
                ...
                "used_percent": used_f if "used_f" in locals() else None,
        ...
        entries.append({
            ...
            "name": info.get("name"),
            "invalid": cred.get("invalid") is True,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Codex quota checker (reads ~/.code-agent/codex-auth.json, cpa-quota style)")
    ap.add_argument("--file", dest="file", default=DEFAULT_FILE, help="path to codex-auth.json (default ~/.code-agent/codex-auth.json)")
