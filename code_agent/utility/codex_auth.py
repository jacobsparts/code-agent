...
def _detect_codex_version() -> str:
    env_v = os.environ.get("CODEX_VERSION")
    if env_v and env_v.strip():
        return env_v.strip()
    for cand in [
        pathlib.Path(__file__).resolve().parent / "codex" / "codex-rs" / "Cargo.toml",
        pathlib.Path(__file__).resolve().parent / "codex" / "Cargo.toml",
        pathlib.Path("/home/jacob/cliproxyapi/src/codex/codex-rs/Cargo.toml"),
        pathlib.Path("/home/jacob/cliproxyapi/src/codex/Cargo.toml"),
    ]:
        try:
            if cand.exists():
                import re as _re
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--cred-file", default=CRED_FILE,
                   help=f"Path to codex-auth.json credentials file (default {CRED_FILE})")
