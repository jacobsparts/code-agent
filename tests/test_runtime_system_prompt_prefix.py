...
def test_conversation_prefix_is_request_time_only():
    ...
    persisted = [dict(message) for message in conversation.messages]
    ...
    assert conversation.messages == persisted
    assert first[0] is not conversation.messages[0]
    assert second[0] is not conversation.messages[0]
...
def test_conversation_ignores_empty_prefix():
    ...
    outgoing = conversation._messages()

    assert outgoing == [{"role": "system", "content": "persisted system"}]
    assert conversation.messages == [{"role": "system", "content": "persisted system"}]
...
def test_repl_direct_mode_uses_existing_direct_python_preamble():
    ...
    assert conversation.messages[0]["content"].startswith("stable base")
    assert "Respond with raw Python only." not in conversation.messages[0]["content"]
...
def test_repl_execute_mode_uses_native_preamble():
    ...
    assert conversation.messages[0]["content"].startswith("stable base")
    assert "Every response must include a repl_execute tool call." not in conversation.messages[0]["content"]
...
def test_production_code_agent_conversation_has_one_runtime_prefix():
    ...
    try:
        ...
        persisted_system = conversation.messages[0]["content"]

        outgoing_system = conversation._messages()[0]["content"]
        ...
        assert conversation.messages[0]["content"] == persisted_system
...
def test_production_model_switch_updates_dispatch_and_runtime_prefix(monkeypatch):
    ...
    try:
        ...
        assert clients["provider/direct"].calls[-1][0]["content"].count(
        ...
        conversation.add_assistant_response()
        native_system = clients["provider/native"].calls[-1][0]["content"]
        ...
        assert conversation.messages[:-1] == persisted
